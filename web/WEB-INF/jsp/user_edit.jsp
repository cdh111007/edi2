...
<html>
    ...
    <body>
        <form  action="${pageContext.request.contextPath}/user/update" method="post" id="user_add_form" onsubmit="return checkinput();">
            <div  class="table-wrapper">
                <table style="height: 80%;
                    ...
                    <thead>
                        <tr>
                            <th>*客户代码：</th>
                            <th><input name="code"  id="code" type="text" style="width: 200px;" value="${user.code}"  readonly="true"/></th>
                        </tr>
                        <tr>
                            <th>*名称：</th>
                        ...
                        <tr>
                            <th>报文格式： </th>
                            <th>
                                <select name="ediType" id="ediType">
                                    <option value="">请选择...</option>
                                    <option value="UN" <c:if test="${user.ediType eq 'UN'}">selected="selected"</c:if>>联合国</option>
                                    <option value="JT" <c:if test="${user.ediType eq 'JT'}">selected="selected"</c:if>>交通部</option>
                                    </select>
                                </th>
                            </tr>
                            <tr>
                                <th>可接收CODECO：</th>
                                <th><input name="codecoFlag" id="codecoFlag" type="checkbox"   value="1"  <c:if test="${user.codecoFlag eq '1'}">checked="checked"</c:if>/></th>
                            </tr>
                            <tr>
                                <th>可接收COARRI：</th>
                                <th><input name="coarriFlag" id="coarriFlag" type="checkbox"   value="1" <c:if test="${user.coarriFlag eq '1'}">checked="checked"</c:if> /></th>
                            </tr>
                            <tr>
                                <th>可接收COEDOR：</th>
