 ...
 */
@Controller
@RequestMapping("/dv_resend")
public class DvCodecoResendController {
    ...
    public String send(HttpServletRequest request, Model model) throws ParseException {
        ...
        if (ValidationUtil.isValid(ctnNos)) {
            return EDIRedo.dv(cstcode, new DateRange(begin, end), ctnNos);
        } else {
            return EDIRedo.dv(cstcode, new DateRange(begin, end));
        }
